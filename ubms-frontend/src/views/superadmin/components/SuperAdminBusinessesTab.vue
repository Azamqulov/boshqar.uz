      ...
      <div v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        ...
        >
          <div class="space-y-3.5">
            <!-- Top header: Initial Icon, Name, Date, Status -->
            <div class="flex items-start justify-between gap-2">
              <div class="flex items-center gap-3">
                <div class="w-11 h-11 rounded-2xl bg-gradient-to-br from-emerald-500/20 to-teal-600/20 text-emerald-600 dark:text-emerald-400 border border-emerald-500/30 flex items-center justify-center font-black text-base shadow-sm group-hover:scale-105 transition-transform">
            ...
            <!-- Owner info card -->
            <div class="p-3 rounded-xl bg-slate-50 dark:bg-slate-800/50 border border-slate-200/60 dark:border-slate-800/60 space-y-1">
              <div class="text-[10px] uppercase font-bold text-slate-400 flex items-center gap-1">
                <Crown class="w-3 h-3 text-amber-500" /> Firma Egasi
              </div>
              <p class="font-bold text-xs text-slate-900 dark:text-white">{{ b.owner?.fullName || '-' }}</p>
              <p class="text-[11px] text-slate-500 dark:text-slate-400 font-mono">{{ b.owner?.phone || '-' }}</p>
            </div>

            <!-- Metrics pill box (Filial, Tovar, Chek) -->
            <div class="grid grid-cols-3 gap-2 text-center">
              <div class="p-2 rounded-xl bg-slate-50 dark:bg-slate-800/40 border border-slate-200/60 dark:border-slate-800/60">
                <span class="text-[10px] text-slate-400 block font-medium">Filiallar</span>
                <span class="font-black text-xs font-mono text-emerald-600 dark:text-emerald-400">{{ b.branchesCount || 0 }}</span>
              </div>
              <div class="p-2 rounded-xl bg-slate-50 dark:bg-slate-800/40 border border-slate-200/60 dark:border-slate-800/60">
                <span class="text-[10px] text-slate-400 block font-medium">Tovarlar</span>
                <span class="font-black text-xs font-mono text-blue-600 dark:text-blue-400">{{ b.productsCount || 0 }}</span>
              </div>
              <div class="p-2 rounded-xl bg-slate-50 dark:bg-slate-800/40 border border-slate-200/60 dark:border-slate-800/60">
                <span class="text-[10px] text-slate-400 block font-medium">Cheklar</span>
                <span class="font-black text-xs font-mono text-teal-600 dark:text-teal-400">{{ b.ordersCount || 0 }}</span>
              </div>
            </div>
          </div>
          ...
          <div class="mt-4 pt-3 border-t border-slate-100 dark:border-slate-800 flex items-center gap-2">
            <button
              @click="$emit('openPlanModal', b)"
              class="flex-1 py-2 px-3 rounded-xl bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 font-bold text-xs transition flex items-center justify-center gap-1.5 btn-interactive"
            >
              <Zap class="w-3.5 h-3.5 text-amber-500" />
              <span>Tarif</span>
            </button>
