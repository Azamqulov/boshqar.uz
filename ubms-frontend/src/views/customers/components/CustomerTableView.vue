<template>
  <div class="w-full">
    ...
    <div class="block md:hidden space-y-3">
      <div v-if="customers.length === 0" class="glass-card rounded-2xl p-8 text-center text-slate-400 dark:text-slate-500">
      ...
      <div
        ...
        :key="c.id"
        class="glass-card rounded-2xl p-4 space-y-3.5 border border-slate-200/80 dark:border-slate-800/80 shadow-xs"
      >
        <!-- Top: Name & Phone & Debt Status -->
        <div class="flex items-start justify-between gap-2">
          ...
          <div class="shrink-0 text-right">
            <span
              class="font-black px-2.5 py-1 rounded-xl text-xs inline-block"
              :class="Number(c.debt) > 0 ? 'bg-rose-500/15 text-rose-600 dark:text-rose-400 border border-rose-500/30' : 'bg-slate-100 dark:bg-slate-800 text-slate-400'"
            >
              {{ Number(c.debt) > 0 ? formatCurrency(c.debt) : 'Qarzsiz' }}
            </span>
          </div>
        </div>

        <!-- Middle: Stats (Purchases & Total Spent) -->
        <div class="grid grid-cols-2 gap-2 p-2.5 rounded-xl bg-slate-50 dark:bg-slate-900/50 border border-slate-100 dark:border-slate-800 text-xs">
          <div>
            <span class="text-[10px] text-slate-400 dark:text-slate-500 block">Xaridlar soni</span>
            <span class="font-bold text-slate-800 dark:text-slate-200 font-mono">{{ c.totalPurchases || 0 }} ta</span>
          </div>
          <div>
            <span class="text-[10px] text-slate-400 dark:text-slate-500 block">Jami xaridi</span>
            <span class="font-bold text-emerald-600 dark:text-emerald-400 font-mono">{{ formatCurrency(c.totalSpent || 0) }}</span>
          </div>
        </div>
        ...
        <div class="flex items-center gap-1.5 pt-1">
          ...
          <button
            ...
            class="flex-1 py-2 px-2.5 rounded-xl bg-rose-500/10 hover:bg-rose-500/20 text-rose-600 dark:text-rose-400 font-bold text-xs transition border border-rose-500/20 flex items-center justify-center gap-1 btn-interactive"
          >
            <Plus class="w-3.5 h-3.5" />
            <span>+ Qarz</span>
          </button>

          <!-- 2. Pay Debt (if has debt) -->
          <button
            v-if="Number(c.debt) > 0"
            type="button"
            @click="$emit('openPayDebt', c)"
            class="flex-1 py-2 px-2.5 rounded-xl bg-emerald-500/15 hover:bg-emerald-500/25 text-emerald-600 dark:text-emerald-400 font-bold text-xs transition border border-emerald-500/30 flex items-center justify-center gap-1 btn-interactive"
          >
          ...
          <button
            ...
            title="Tarix"
            class="p-2 rounded-xl text-slate-500 hover:text-slate-700 dark:hover:text-white bg-slate-100 dark:bg-slate-800 transition"
          >
          ...
          <button
            ...
            @click="$emit('delete', c)"
            title="O'chirish"
            class="p-2 rounded-xl text-slate-400 hover:text-rose-500 bg-rose-500/10 transition"
          >
