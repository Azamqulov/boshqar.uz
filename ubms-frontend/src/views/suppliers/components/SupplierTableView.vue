<template>
  <div class="w-full">
    ...
    <div class="block md:hidden space-y-3">
      <div v-if="suppliers.length === 0" class="glass-card rounded-2xl p-8 text-center text-slate-400 dark:text-slate-500">
      ...
      >
        ...
        <div class="flex items-start justify-between gap-2">
          <div class="min-w-0">
            ...
            <p v-if="s.companyName" class="text-xs text-slate-500 dark:text-slate-400 truncate mt-0.5">
              {{ s.companyName }}
            </p>
          </div>
          ...
          <!-- Balance Badge -->
          <div
            class="font-black inline-flex items-center gap-1 px-2.5 py-1 rounded-xl text-xs shrink-0 font-mono"
            :class="[
              Number(s.balance) > 0
          ...
          >
            ...
            <span v-if="Number(s.balance) > 0" class="text-[9px] uppercase font-bold">(Qarz)</span>
            <span v-else-if="Number(s.balance) < 0" class="text-[9px] uppercase font-bold">(Haq)</span>
          </div>
        </div>
        ...
        <div class="grid grid-cols-2 gap-2 p-2.5 rounded-xl bg-slate-50 dark:bg-slate-900/50 border border-slate-100 dark:border-slate-800 text-xs">
          <div>
            <span class="text-[10px] text-slate-400 dark:text-slate-500 block">Telefon</span>
            <a v-if="s.phone" :href="`tel:${s.phone}`" class="font-bold text-blue-600 dark:text-blue-400 font-mono truncate block">
              {{ s.phone }}
            </a>
            <span v-else class="text-slate-400">—</span>
          </div>
          <div>
            <span class="text-[10px] text-slate-400 dark:text-slate-500 block">Manzil</span>
            <span class="font-medium text-slate-700 dark:text-slate-300 truncate block">{{ s.address || '—' }}</span>
          </div>
        </div>
        ...
        <div class="flex items-center gap-1.5 pt-1">
          ...
          <button
            ...
            class="flex-1 py-2 px-2.5 rounded-xl bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-600 dark:text-emerald-400 text-xs font-bold transition flex items-center justify-center gap-1 btn-interactive"
          >
          ...
          <button
            ...
            class="p-2 rounded-xl text-emerald-600 bg-emerald-50 dark:bg-emerald-950/40 transition"
            title="Akt Sverka"
          >
...
import { Truck, Building2, Phone, MapPin, AlertCircle, CreditCard, FileSpreadsheet, History, Edit, Trash2 } from 'lucide-vue-next';
