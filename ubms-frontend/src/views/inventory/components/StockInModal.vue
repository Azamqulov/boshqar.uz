      ...
      <div class="modal-body overflow-visible">
        <form @submit.prevent="$emit('submit')" class="space-y-3.5 text-xs">
          ...
          <div class="grid grid-cols-2 gap-3">
            <div>
              ...
              <div class="relative flex items-center">
                <input
                  ...
                  placeholder="0"
                  required
                  class="w-full px-3 py-2.5 pr-14 text-xs rounded-xl bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 text-slate-900 dark:text-white font-bold focus:ring-2 focus:ring-emerald-500 transition shadow-inner"
                />
                <span class="absolute right-3 px-1.5 py-0.5 rounded bg-slate-100 dark:bg-slate-800 text-[10px] font-black text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-700 pointer-events-none uppercase">
                  {{ currentUnit }}
                </span>
              </div>
            </div>
            <div>
              <label class="block font-bold text-slate-700 dark:text-slate-300 mb-1.5">
                Kirim Narxi (1 {{ currentUnit }} uchun)
              </label>
              <CurrencyInput
...
const selectedItem = computed(() => {
  return props.inventoryInOptions.find((opt) => opt.value === props.stockForm.productId);
});
