<template>
  ...
  >
    <div
      ...
      class="fixed bottom-20 sm:bottom-6 left-4 right-4 sm:left-auto sm:right-6 sm:max-w-md z-40 p-4 rounded-2xl bg-slate-900/95 dark:bg-slate-900/95 text-white border border-emerald-500/30 shadow-2xl backdrop-blur-xl space-y-3"
    >
        ...
        <div class="flex items-center gap-3">
          ...
          <div>
            <h4 class="text-xs sm:text-sm font-bold text-white flex items-center gap-1.5">
              ...
              <span class="px-1.5 py-0.5 rounded-full text-[9px] font-extrabold bg-emerald-500 text-slate-950 uppercase">PWA</span>
            </h4>
      ...
      <div class="flex items-center justify-end gap-2 pt-1">
        <button
          @click="dismiss"
          class="px-3 py-1.5 rounded-xl text-xs font-semibold text-slate-400 hover:text-white hover:bg-slate-800 transition"
        >
          Keyinroq
        </button>
        <button
          @click="installPwa"
          class="px-3.5 py-1.5 rounded-xl text-xs font-bold bg-emerald-500 hover:bg-emerald-400 text-slate-950 shadow-md shadow-emerald-500/25 transition flex items-center gap-1.5 hover:scale-105 active:scale-95"
        >
